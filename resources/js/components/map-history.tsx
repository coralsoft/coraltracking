...
export default function MapHistory({ positions, vehicleName, selectedIndex, onSelectIndex, routes = [] }: MapHistoryProps) {
    ...
    return (
        <div className="h-full w-full min-h-[400px] rounded-xl overflow-hidden border border-sidebar-border/70 dark:border-sidebar-border">
            ...
            >
                ...
                {hasMultiRoutes && routes.map((route) => {
                    const routeLatLngs = route.positions.map((p) => [p.latitude, p.longitude] as [number, number]);
                ...
                {hasSingleRoute && (
                    <>
                        <Polyline positions={latLngs} pathOptions={{ color: '#c8246b', weight: 4 }} />
                        {latLngs[0] && (
